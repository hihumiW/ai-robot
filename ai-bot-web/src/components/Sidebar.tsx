...
const textTransition = 'overflow-hidden whitespace-nowrap transition-[opacity,max-width] duration-200 ease-out';
...
export default defineComponent({
  ...
  setup() {
    ...
    return () => (
      ...
      >
        <header class={clsx('flex h-16 items-center px-4', collapsed.value ? 'justify-center' : 'justify-between')}>
          {
            !collapsed.value && <div class={['flex min-w-0 items-center gap-2']}>
            <div class="min-w-0">
              <div class="truncate text-sm font-semibold tracking-wide">AI Bot</div>
          ...
          >
            {collapsed.value ? <PanelLeftOpen size={18} /> : <PanelLeftClose size={18} />}
          </Button>
        </header>
        ...
        <section class={clsx('mt-5 flex min-h-0 flex-1 flex-col px-3',  collapsed.value ? 'opacity-0' : 'opacity-100')}>
          <div
            class="mb-2 px-1 text-xs font-medium text-zinc-500 transition-opacity duration-150"
          >
            会话历史
          </div>
          <div class="scrollbar-thin min-h-0 flex-1 space-y-1 overflow-y-auto pr-1">
            {conversations.map((title, index) => (
              <Button key={title} class={itemClass(index === 0)} title={title}>
              
                <span class={['truncate text-left', textClass()]}>{title}</span>
              </Button>
            ))}
          </div>
        </section>
